...
#------------------读取txt文本文件中的数据--------------------
user_income = pd.read_table(r'/Users/hanxu/Documents/My Document/学习资料/从零开始学Python--数据分析与挖掘/第5章 Python数据处理工具--Pandas/data_test01.txt', sep = ',', 
                            parse_dates={'birthday':[0,1,2]},skiprows=2, skipfooter=3, 
                            comment='#', encoding='utf8', thousands='&')
user_income
# 读取excel电子表格数据
child_cloth = pd.read_excel(io = r'/Users/hanxu/Documents/My Document/学习资料/从零开始学Python--数据分析与挖掘/第5章 Python数据处理工具--Pandas/data_test02.xlsx', header = None, 
...
#查询数据
findresult = collection.find_one({'name':'Li'})
print(findresult)
print(type(result))

findresults = collection.find()
print(findresults)
for x in findresults:
    print(x)
...
#------------------5.3　数据类型转换及描述统计-------------------
# 数据读取，read_table读取csv或txt
sec_cars = pd.read_table(r'/Users/hanxu/Documents/My Document/学习资料/从零开始学Python--数据分析与挖掘/第5章 Python数据处理工具--Pandas/sec_cars.csv', sep = ',')
# 预览数据的前五行
sec_cars.head()
# 查看数据的行列数
print('数据集的行列数：\n',sec_cars.shape)
...
# 修改二手车上牌时间的数据类型，to_datetime用来转成日期格式
sec_cars.Boarding_time = pd.to_datetime(sec_cars.Boarding_time, format = '%Y年%m月')
# 修改二手车新车价格的数据类型，str转成字符串格式然后[]切掉最后的万字，最后astype转数据类型
...
# 数据的描述性统计
sec_cars.describe()
# 数据的形状特征，下面这段用到时候需要重点看。columns用于返回变量名，apply可以指定计算方向（axis=0是垂直的各列）
# 挑出所有数值型变量
...
#-----------------5.4 字符与日期数据的处理----------------
# 数据读入
df = pd.read_excel(r'/Users/hanxu/Documents/My Document/学习资料/从零开始学Python--数据分析与挖掘/第5章 Python数据处理工具--Pandas/data_test03.xlsx')
# 各变量数据类型
print(df.dtypes)
# 将birthday变量转换为日期型
