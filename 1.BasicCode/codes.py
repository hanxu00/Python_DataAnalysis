...
#练习：取出所有日期
print(re.findall("ymd:'(\d*-\d*-\d*)",string8))
...
#正则表达式貌似是将字符串顺序遍历匹配
print(re.findall('[，。、a-zA-Z0-9（）]',string10))
...
dict3 = {}
# 循环计算
for i in set(list6):  #set是干什么的？ 把list转化成set？为啥要这样？
...
def game(min,max):
    ...
    while True:
        guess = float(input('请在'+min+'到'+max+'之间猜一个数字: '))

        if guess < number:
            min = guess
            print('不好意思，你猜的的数偏小了！请在%d到%d之间猜一个数！' %(min,max))  #%占位符使用
        elif guess > number:
            max = guess
